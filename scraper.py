...
def search_for_movie(title, year, handle, settings):
    ...
    if year is not None:
        ...
        if not search_results:
            log("Find movie with umlaut title '{title}' from year '{year}'".format(title=title_umlaut, year=year), xbmc.LOGINFO)
            search_results = scraper.search(title_umlaut, year)
        if not search_results:
            log("Find movie with umlaut title '{title}' from year '{year}'".format(title=title_umlaut, year="xxx"), xbmc.LOGINFO)
